...
int sumuj(int ilosc_procesow){ 
    ...
    // 1. Tworzenie procesów potomnych (fork)
    ...
    int shmid_wektor_parent = shmget(key_wektor, size_wektor, 0666 | IPC_CREAT);
    if (shmid_wektor_parent == -1) {
        perror("blad shmget wektor");
        return 1;
    }

    // 5. Uzupełnianie wektora w pamieci współdzielonej
    vector_shm = (double*) shmat(shmid_wektor_parent, NULL, 0);
    ...
    for (int i = 0; i < n; i++) {
        if (fscanf(f, "%lf", &vector_shm[i]) != 1) {
            ...
            // Sprzątanie
            shmdt(vector_shm);
            shmctl(shmid_wektor_parent, IPC_RMID, NULL);
            return 1;
        }
    }
    fclose(f);
    ...
    if(debug){
        printf("Końcowa suma elementów w wektorze = %f\n", suma_koncowa);
    }
    ...
    if (shmctl(shmid_wektor_parent, IPC_RMID, NULL) == -1) {
...
int main(){
    int ilosc_procesow[] = {1,2,4,6,8,16};
    int powtorzenia = 1000;

    struct timespec start, end;

    for(int i = 0; i < 6; i++){
        int n = ilosc_procesow[i];
        ...
        double laczny = ((end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9) - (ilosc_procesow[i]*0.005);
        double sredni = (laczny / powtorzenia);

        printf("Ilosc procesow: %d, czas: %f, sredni czas: %f, wynik: %.2f\n", n, laczny, sredni, wynik);
